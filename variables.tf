...
variable "khulnasoft_aws_account_id" {
  ...
  type        = string
  validation {
    condition     = can(regex("^\\d{12}$", var.khulnasoft_aws_account_id))
    error_message = "Khulnasoft AWS account ID must be a 12-digit number"
  }
}
...
variable "khulnasoft_volscan_api_token" {
  ...
  type        = string
  validation {
    condition     = length(var.khulnasoft_volscan_api_token) > 0
    error_message = "Khulnasoft Volume Scanning API Token must not be empty"
  }
}
...
variable "khulnasoft_volscan_api_url" {
  ...
  type        = string
  validation {
    condition     = can(regex("^https?://", var.khulnasoft_volscan_api_url))
    error_message = "Khulnasoft Volume Scanning API URL must start with or 'https://'"
  }
}
...
variable "create_role_name" {
  ...
  type        = string
  default     = "KhulnasoftAutoConnectAgentlessRole"
  validation {
...
variable "identity_pool_name" {
  description = "Name of the identity pool. If not provided, the default value is set to 'khulnasoft-agentless-pool-<khulnasoft_tenant_id>' in the 'identity_pool_name' local"
  type        = string
  default     = null
}

variable "identity_pool_provider_name" {
  description = "Name of the identity pool provider. If not provided, the default value is set to 'agentless-provider-<khulnasoft_tenant_id>' in the 'identity_pool_provider_name' local"
  type        = string
  default     = null
}

variable "service_account_name" {
  description = "Name of the service account. If not provided, the default value is set to 'khulnasoft-agentless-sa-<khulnasoft_tenant_id>' in the 'service_account_name' local"
  type        = string
  default     = null
}
