...
resource "google_iam_workload_identity_pool" "workload_identity_pool" {
  display_name              = "Khulnasoft agentless pool"
  description               = "Khulnasoft agentless pool"
...
resource "google_iam_workload_identity_pool_provider" "workload_identity_pool_provider" {
  ...
  aws {
    account_id = var.khulnasoft_aws_account_id
  }
...
# Create a custom IAM role for Khulnasoft CSPM for dedicated organization onboarding
...
# Create a service account for Khulnasoft CSPM for dedicated organization onboarding
#trivy:ignore:AVD-GCP-0011
resource "google_service_account" "cspm_service_account" {
  count        = var.type == "organization" && var.dedicated_project ? 1 : 0
  account_id   = "khulnasoft-cspm-scanner-${var.khulnasoft_tenant_id}"
  display_name = "khulnasoft-cspm-scanner-${var.khulnasoft_tenant_id}"
...
resource "google_project_iam_binding" "project_iam_binding_delete_role" {
  ...
  condition {
    title       = "Khulnasoft Resource Delete Condition"
    description = "Condition for Khulnasoft delete role to delete khulnasoft resources only"
