...
data "external" "gcp_onboarding" {
  ...
  query = {
    api_key                  = sensitive(var.khulnasoft_api_key)
    api_secret               = sensitive(var.khulnasoft_api_secret)
    autoconnect_url          = var.khulnasoft_autoconnect_url
    service_account_key      = local.service_account_key
    client_config            = local.client_config_rendered
    cspm_group_id            = var.khulnasoft_cspm_group_id
    project_id               = var.project_id
    configuration_id         = var.khulnasoft_configuration_id
