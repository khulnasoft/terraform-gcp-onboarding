...
# Bind the Khulnasoft service account to the CSPM role
...
# Create a custom IAM role for Khulnasoft CSPM
resource "google_project_iam_custom_role" "cspm_role" {
  ...
  description = "Khulnasoft AutoConnect Role"
  permissions = tolist(local.cspm_role_permissions)
  project     = var.project_id
}

# Generate a random integer for the Khulnasoft service account ID only for organization same and single onboarding
...
resource "google_service_account" "cspm_service_account" {
  count        = var.type == "organization" ? 0 : 1
  account_id   = "khulnasoft-${random_integer.cspm_service_account_id[0].result}"
  display_name = "khulnasoft-${random_integer.cspm_service_account_id[0].result}"
